...
async def handle_pika_webhook(bot: Bot, dp: Dispatcher, body: dict):
    ...
    if not is_generations_success:
        ...
        if "content violation" in error_message:
            ...
            logging.exception("Error in pika_webhook", error_message)
            await bot.send_sticker(
                chat_id=user.telegram_chat_id,
    ...
    if len(generations_result) > 0:
        ...
        footer_text = (
            ...
            if user.settings[Model.PIKA][UserSettings.SHOW_USAGE_QUOTA]
            and user.daily_limits[Quota.PIKA] != float("inf")
            else ""
        )
