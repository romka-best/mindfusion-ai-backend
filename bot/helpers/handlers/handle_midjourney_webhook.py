...
async def handle_midjourney_webhook(bot: Bot, dp: Dispatcher, body: dict):
    ...
    if generation_error or not generation_result:
        ...
        if not generation.details.get("is_suggestion", False):
            if (
                "might be against our community standards" in generation_error
                or "Banned prompt detected" in generation_error
            ):
    ...
    asyncio.create_task(
        handle_midjourney_result(bot, dp, user, user_language_code, request, generation)
    )
...
async def handle_midjourney_result(
    ...
    if not generation.has_error and not is_suggestion:
        reply_markup = (
            build_midjourney_keyboard(generation.id)
            if action_type != MidjourneyAction.UPSCALE
        ...
        footer_text = (
            ...
            if action_type != MidjourneyAction.UPSCALE
            and user.settings[Model.MIDJOURNEY][UserSettings.SHOW_USAGE_QUOTA]
            and user.daily_limits[Quota.MIDJOURNEY] != float("inf")
            else ""
        )
        middle_text = (
            get_localization(user_language_code).MIDJOURNEY_INFO
            if action_type != MidjourneyAction.UPSCALE
            else ""
        )
    ...
    price = Midjourney.get_price_for_image(
        generation.details.get("version"), generation.details.get("action")
    )
    update_tasks = [
        write_transaction(
            user_id=user.id,
    ...
    if (
        not generation.has_error
        and not is_suggestion
        and action_type != MidjourneyAction.UPSCALE
    ):
