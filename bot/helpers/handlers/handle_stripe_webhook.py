...
async def handle_stripe_webhook(request: dict, bot: Bot, dp: Dispatcher):
    ...
    try:
        ...
        if subscription is not None and (
            ...
            or subscription.status == SubscriptionStatus.DECLINED
        ):
            ...
            old_subscription = (
                await get_subscription_by_provider_auto_payment_charge_id(order_id)
            )
            if old_subscription is not None and (
                old_subscription.status == SubscriptionStatus.ACTIVE
                or old_subscription.status == SubscriptionStatus.TRIAL
                or old_subscription.status == SubscriptionStatus.FINISHED
            ):
                ...
                if request_type == "invoice.payment_succeeded":
                    ...
                    if old_subscription.status == SubscriptionStatus.TRIAL:
                        new_income_amount = old_subscription.income_amount + float(
                            clear_amount
                        )
                    ...
                    await update_subscription(
                        old_subscription.id, {"status": old_subscription.status}
                    )
                    await update_user(
    ...
    try:
        ...
        if len(packages) == 1:
            ...
            if request_type == "payment_intent.succeeded":
                ...
                request_type == "payment_intent.payment_failed"
                or request_type == "payment_intent.canceled"
            ):
