...
async def delayed_send_message_to_super_admin(
    bot: Bot, text: str, parse_mode: str, timeout: int
):
    ...
    try:
        ...
        asyncio.create_task(
            delayed_send_message_to_super_admin(
                bot, text, parse_mode, e.retry_after + 30
            )
        )
    except (
        ...
        logging.exception(
            f"Error in delayed_send_message_to_super_admin: {error_trace}"
        )
...
async def send_message_to_super_admin(bot: Bot, message: str, parse_mode="HTML"):
    try:
        ...
        asyncio.create_task(
            delayed_send_message_to_super_admin(
                bot, message, parse_mode, e.retry_after + 30
            )
        )
    except (
        ConnectionResetError,
