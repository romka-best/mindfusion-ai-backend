...
from bot.database.models.common import PaymentMethod, Quota
...
async def update_daily_limits(bot: Bot, storage: BaseStorage):
    ...
    await send_message_to_admins_and_developers(
        bot, "<b>Updated Daily Limits Successfully</b> 🎉"
    )


async def update_user_daily_limits(
    bot: Bot, user: User, batch: AsyncWriteBatch, storage: BaseStorage
):
    try:
        user = await update_user_subscription(bot, user, batch, storage)
        await update_user_additional_usage_quota(
            bot, user, bool(user.subscription_id), batch, storage
        )
...
async def update_user_subscription(
    ...
    if (
        ...
        and current_subscription.status == SubscriptionStatus.TRIAL
        and (current_date - current_subscription.start_date).days >= 3
    ):
        ...
        if current_subscription.payment_method == PaymentMethod.YOOKASSA:
            ...
            if not payment:
                ...
                activated_subscriptions = await get_activated_subscriptions_by_user_id(
                ...
                for activated_subscription in activated_subscriptions:
                    if activated_subscription.id != current_subscription.id:
                        ...
                        user.daily_limits = activated_subscription_product.details.get(
                            "limits"
                        )
                        break
                else:
    ...
    if (
        current_subscription
        and current_subscription.status != SubscriptionStatus.FINISHED
        and current_subscription.end_date <= current_date
    ):
        user_language_code = await get_user_language(user.id, storage)
        if (
            current_subscription.provider_auto_payment_charge_id
            and current_subscription.status != SubscriptionStatus.CANCELED
        ):
            product = await get_product(current_subscription.product_id)
            if current_subscription.payment_method == PaymentMethod.YOOKASSA:
                payment = await create_auto_payment(
                ...
                if not payment:
                    ...
                    await update_subscription(
                        current_subscription.id, {"status": current_subscription.status}
                    )
                    batch.update(
                        user_ref,
                        {
                ...
                current_subscription.payment_method == PaymentMethod.STRIPE
                or current_subscription.payment_method == PaymentMethod.TELEGRAM_STARS
            ):
...
async def update_user_additional_usage_quota(
    ...
    if need_update:
        ...
        for package in packages:
            ...
            if (
                product.details.get("is_recurring", False)
                and package.until_at > current_date
            ):
        ...
        if not user.additional_usage_quota[Quota.VOICE_MESSAGES]:
            user.settings[user.current_model][UserSettings.TURN_ON_VOICE_MESSAGES] = (
                False
            )
        ...
        if (
            not had_subscription
            and count_active_packages_before > count_active_packages_after
        ):
            user_language_code = await get_user_language(user.id, storage)
            await send_sticker(
                bot,
