...
async def create_face_swap_images(images: list[dict]):
    tasks = [
        create_face_swap_image(image["target_image"], image["source_image"])
        for image in images
    ]
...
async def create_photoshop_ai_image(
    action: PhotoshopAIAction, image_url: str
) -> Optional[str]:
    if action == PhotoshopAIAction.UPSCALE:
        ...
        model = await replicate.models.async_get(
