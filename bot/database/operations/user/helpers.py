...
def create_user_object(
    ...
    return User(
        ...
        balance=user_data.get("balance", 100 if is_referred_by_user else 75),
        subscription_id=user_data.get("subscription_id", ""),
        last_subscription_limit_update=user_data.get(
            "last_subscription_limit_update", datetime.now(timezone.utc)
        ),
        daily_limits=user_data.get("daily_limits", SUBSCRIPTION_FREE_LIMITS),
        additional_usage_quota=user_data.get(
            "additional_usage_quota", default_additional_quota
        ),
