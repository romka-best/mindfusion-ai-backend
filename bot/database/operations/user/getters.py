...
async def get_users(
    ...
    if is_blocked is not None:
        users_query = users_query.where(
            filter=FieldFilter("is_blocked", "==", is_blocked)
        )
    if utm is not None:
        if utm.get(UTM.SOURCE):
            users_query = users_query.where(
                filter=FieldFilter("utm.source", "==", utm.get(UTM.SOURCE))
            )
        if utm.get(UTM.MEDIUM):
            users_query = users_query.where(
                filter=FieldFilter("utm.medium", "==", utm.get(UTM.MEDIUM))
            )
        if utm.get(UTM.CAMPAIGN):
            users_query = users_query.where(
                filter=FieldFilter("utm.campaign", "==", utm.get(UTM.CAMPAIGN))
            )
        if utm.get(UTM.TERM):
            users_query = users_query.where(
                filter=FieldFilter("utm.term", "==", utm.get(UTM.TERM))
            )
        if utm.get(UTM.CONTENT):
            users_query = users_query.where(
                filter=FieldFilter("utm.content", "==", utm.get(UTM.CONTENT))
            )
...
async def get_count_of_users_referred_by(
    ...
    users_query = (
        await users_query.where(filter=FieldFilter("referred_by", ">=", ""))
...
async def get_count_of_users_campaign_by(
    ...
    users_query = (
        await users_query.where(filter=FieldFilter("utm.campaign", ">=", ""))
        .count()
        .get()
    )
...
async def get_count_of_users_by_referral(referred_by: str) -> int:
    users_query = (
        await firebase.db.collection(User.COLLECTION_NAME)
...
async def get_users_by_language_code(language_code: LanguageCode) -> list[User]:
    users_stream = (
        firebase.db.collection(User.COLLECTION_NAME)
        .where(filter=FieldFilter("interface_language_code", "==", language_code))
        .stream()
    )

    return [User(**user.to_dict()) async for user in users_stream]
