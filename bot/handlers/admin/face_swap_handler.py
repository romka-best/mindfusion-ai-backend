...
async def handle_face_swap_manage_selection(
    ...
    if action == "back":
        ...
        await callback_query.message.edit_text(
            text=get_localization(
                user_language_code
            ).ADMIN_FACE_SWAP_EDIT_CHOOSE_GENDER,
            reply_markup=build_manage_face_swap_edit_choose_gender_keyboard(
                user_language_code
            ),
        )
...
async def handle_face_swap_manage_create_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
@admin_face_swap_router.message(
    FaceSwap.waiting_for_face_swap_system_package_name, F.text, ~F.text.startswith("/")
)
async def face_swap_manage_system_name_sent(message: Message, state: FSMContext):
    ...
    if face_swap_package_male or face_swap_package_female:
        ...
        await state.update_data(
            system_face_swap_package_name=system_face_swap_package_name
        )
        await state.set_state(FaceSwap.waiting_for_face_swap_package_name)


@admin_face_swap_router.message(
    FaceSwap.waiting_for_face_swap_package_name, F.text, ~F.text.startswith("/")
)
async def face_swap_manage_name_sent(message: Message, state: FSMContext):
    ...
    for language_code in localization_classes.keys():
        if language_code == LanguageCode.RU:
            ...
            translated_face_swap_package_name = await translate_text(
                message.text, LanguageCode.RU, language_code
            )
            if translated_face_swap_package_name:
                face_swap_package_names[language_code] = (
                    translated_face_swap_package_name
                )
            else:
                face_swap_package_names[language_code] = message.text
    ...
    await message.answer(
        text=get_localization(
            user_language_code
        ).admin_face_swap_create_package_confirmation(
            package_system_name=user_data["system_face_swap_package_name"],
            package_names=face_swap_package_names,
        ),
        reply_markup=build_manage_face_swap_create_confirmation_keyboard(
            user_language_code
        ),
    )
...
@admin_face_swap_router.callback_query(
    lambda c: c.data.startswith("fsm_create_confirmation:")
)
async def handle_face_swap_manage_create_confirmation_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    if action == "approve":
        ...
        await handle_manage_face_swap(
            answered_message, str(callback_query.from_user.id), state
        )
...
@admin_face_swap_router.callback_query(
    lambda c: c.data.startswith("fsm_edit_choose_gender:")
)
async def handle_face_swap_manage_edit_choose_gender_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    await callback_query.message.edit_text(
        ...
        reply_markup=build_manage_face_swap_edit_choose_package_keyboard(
...
@admin_face_swap_router.callback_query(
    lambda c: c.data.startswith("fsm_edit_choose_package:")
)
async def handle_face_swap_manage_edit_choose_package_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def show_picture(
    ...
    try:
        ...
        await callback_query.message.answer_photo(
            ...
            reply_markup=build_manage_face_swap_edit_picture_keyboard(
...
async def handle_face_swap_manage_edit_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    if action == "back":
        ...
        if action == "change_status":
            ...
            await callback_query.message.edit_text(
                text=get_localization(
                    user_language_code
                ).ADMIN_FACE_SWAP_ADD_NEW_PICTURE_NAME,
                reply_markup=build_cancel_keyboard(user_language_code),
            )
...
@admin_face_swap_router.message(
    FaceSwap.waiting_for_face_swap_picture_name, ~F.text.startswith("/")
)
...
@admin_face_swap_router.callback_query(
    lambda c: c.data.startswith("fsm_edit_package_change_status:")
)
async def handle_face_swap_manage_edit_package_change_status_selection(
    ...
    if status == "back":
        await handle_face_swap_manage_edit_choose_package_selection(
    ...
    if keyboard_changed:
        await update_face_swap_package(
            user_data["face_swap_package_id"], {"status": status}
        )
...
async def handle_face_swap_manage_edit_picture_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    if action == "change_status":
        await callback_query.message.edit_reply_markup(
            reply_markup=build_manage_face_swap_edit_picture_change_status_keyboard(
                user_language_code, file_status
            ),
        )
        ...
        processing_message = await callback_query.message.reply(
            text=get_localization(
                user_language_code
            ).model_face_swap_processing_request(),
        ...
        async with ChatActionSender.upload_photo(
            ...
            face_swap_response = await create_face_swap_image(
                image_link, user_photo_link
            )
...
@admin_face_swap_router.callback_query(
    lambda c: c.data.startswith("fsm_edit_picture_change_status:")
)
async def handle_face_swap_manage_edit_picture_change_status_selection(
    ...
    if status == "back":
        await callback_query.message.edit_reply_markup(
            reply_markup=build_manage_face_swap_edit_picture_keyboard(
                user_language_code, user_data["file_name"]
            )
        )
        return
    ...
    if keyboard_changed:
        await update_face_swap_package(
            user_data["face_swap_package_id"], {"files": face_swap_package.files}
        )
        await callback_query.message.edit_reply_markup(
            reply_markup=InlineKeyboardMarkup(inline_keyboard=new_keyboard)
        )
