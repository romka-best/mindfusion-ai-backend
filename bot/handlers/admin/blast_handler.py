...
async def handle_blast_user_type_selection(
...
async def handle_blast_language_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def blast_letter_sent(message: Message, state: FSMContext):
    ...
    if blast_language != "all":
        ...
        for language_code in localization_classes.keys():
            if language_code == LanguageCode.RU:
                ...
                translated_blast_letter = await translate_text(
    ...
    await message.answer(
        text=get_localization(user_language_code).admin_blast_confirmation(
            blast_letters
        ),
...
async def handle_blast_confirmation_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    if action == "approve":
        ...
        await callback_query.message.answer(
            text=get_localization(user_language_code).ADMIN_BLAST_SUCCESS
        )
