...
from bot.database.models.transaction import ServiceType, Transaction, TransactionType
from bot.database.operations.feedback.getters import get_count_of_feedbacks
from bot.database.operations.game.getters import (
    get_count_of_games,
...
async def handle_statistics(message: Message, user_id: str, state: FSMContext):
    ...
    await message.edit_text(
        ...
        reply_markup=build_statistics_keyboard(
            user_language_code, is_admin(str(user_id))
        ),
    )


async def handle_write_transaction(
    callback_query: CallbackQuery, language_code: LanguageCode
):
...
async def get_statistics_by_transactions_query(
    ...
    count_all_transactions[ServiceType.DATABASE] = (
        default_transaction_nested_dict.copy()
    )
    ...
    service_subscriptions = [
        product.id for product in products if product.type == ProductType.SUBSCRIPTION
    ]
    service_ai_models = [
        product.id
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category != ProductCategory.OTHER
    ]
    ...
    transactions_query = firebase.db.collection(Transaction.COLLECTION_NAME).order_by(
        "created_at"
    )
    ...
    while is_running:
        ...
        async for doc in docs:
            ...
            if not transaction_user.subscription_id:
                ...
                transaction_user_subscription = await get_subscription(
                    transaction_user.subscription_id
                )
                subscription_users[transaction_user_subscription.product_id].add(
                    transaction_user.id
                )
            ...
            if transaction.type == TransactionType.INCOME:
                ...
                count_all_transactions[transaction.product_id]["BONUS"] += (
                    1 if transaction.details.get("is_bonus", False) else 0
                )
                ...
                count_all_transactions[transaction.product_id]["SUCCESS"] += (
                    transaction.quantity if not has_error else 0
                )
                count_all_transactions[transaction.product_id]["FAIL"] += (
                    transaction.quantity if has_error else 0
                )
                count_all_transactions[transaction.product_id]["EXAMPLE"] += (
                    transaction.quantity if is_suggestion else 0
                )
                count_all_transactions[transaction.product_id]["ALL"] += (
                    transaction.quantity
                )

                count_expense_money[transaction.product_id][
                    "AVERAGE_EXAMPLE_PRICE"
                ] += transaction.amount if is_suggestion else 0
                count_expense_money[transaction.product_id]["EXAMPLE_ALL"] += (
                    transaction.amount if is_suggestion else 0
                )
                count_expense_money[transaction.product_id]["AVERAGE_PRICE"] += (
                    transaction.amount if not is_suggestion else 0
                )
                    ...
                    for key, value in subscription_users.items():
                        if transaction.user_id in value:
                            if transaction.product_id in text_products:
                                ...
                                count_expense_money[key]["SUMMARY"] += (
                                    transaction.amount
                                )
    ...
    for service in service_ai_models:
        ...
        count_expense_money[service]["AVERAGE_PRICE"] = (
            average_price / total if total > 0 else 0
        )
        count_expense_money[service]["AVERAGE_EXAMPLE_PRICE"] = (
            average_example_price / examples if examples > 0 else 0
        )
    for key, value in subscription_users.items():
        count_expense_money[key]["AVERAGE_PRICE"] = (
            (count_expense_money[key]["ALL"] / len(value)) if len(value) else 0
        )
...
async def handle_get_statistics(language_code: LanguageCode, period: str):
    ...
    if period == "day":
        ...
        start_date_before = start_date - timedelta(
            days=calendar.monthrange(start_date.year, start_date.month)[1] - 1
        )
    ...
    text_products = {
        product.id: product.names.get(language_code)
        for product in products
    ...
    text_package_products = {
        ...
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category == ProductCategory.TEXT
    }
    summary_package_products = {
        product.id: product.names.get(language_code)
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category == ProductCategory.SUMMARY
    }
    image_package_products = {
        product.id: product.names.get(language_code)
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category == ProductCategory.IMAGE
    }
    music_package_products = {
        product.id: product.names.get(language_code)
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category == ProductCategory.MUSIC
    }
    video_package_products = {
        product.id: product.names.get(language_code)
        for product in products
        if product.type == ProductType.PACKAGE
        and product.category == ProductCategory.VIDEO
    }
    ...
    tech_products = {
        ...
        for product in products
        if product.type == ProductType.PACKAGE
        and "voice answers" in product.names.get(LanguageCode.EN, "").lower()
    }
    ...
    subscription_results, subscription_before_results = await asyncio.gather(
        asyncio.gather(*subscription_tasks), asyncio.gather(*subscription_before_tasks)
    )

    count_subscription_users[ServiceType.FREE] = 0
    count_subscription_users_before[ServiceType.FREE] = 0
    for subscription, count, count_before in zip(
        subscriptions, subscription_results, subscription_before_results, strict=True
    ):
    ...
    count_subscription_users[ServiceType.FREE] = (
        abs(count_users - sum(count_subscription_users.values()))
        if count_subscription_users[ServiceType.FREE] == 0
        else count_subscription_users[ServiceType.FREE]
    )
    count_subscription_users_before[ServiceType.FREE] = (
        abs(count_users - sum(count_subscription_users_before.values()))
    ...
    for product_with_reactions in products_with_reactions:
        for generation_reaction in [
            GenerationReaction.LIKED,
            GenerationReaction.DISLIKED,
            GenerationReaction.NONE,
        ]:
            count_reactions[product_with_reactions][
                generation_reaction
            ] = await get_count_of_generations(
                start_date=start_date,
                end_date=end_date,
                reaction=generation_reaction,
                product_id=product_with_reactions,
            )
            count_reactions_before[product_with_reactions][generation_reaction] = (
                await get_count_of_generations(
                    start_date=start_date,
                    end_date=end_date,
                    reaction=generation_reaction,
    ...
    count_games = dict.fromkeys(GameType, 0)
    count_games_before = dict.fromkeys(GameType.keys(), 0)
    ...
    count_activated_promo_codes = await get_count_of_used_promo_codes(
        start_date, end_date
    )
    count_activated_promo_codes_before = (
        await get_count_of_used_promo_codes(start_date_before, end_date_before)
        if start_date_before and end_date_before
        else []
    )
    ...
    all_feedbacks_before = (
        await get_count_of_feedbacks(start_date_before, end_date_before)
        if start_date_before and end_date_before
        else 0
    )
    approved_feedbacks = await get_count_of_feedbacks(
        start_date, end_date, FeedbackStatus.APPROVED
    )
    approved_feedbacks_before = (
        await get_count_of_feedbacks(
            start_date_before, end_date_before, FeedbackStatus.APPROVED
        )
        if start_date_before and end_date_before
        else 0
    )
    denied_feedbacks = await get_count_of_feedbacks(
        start_date, end_date, FeedbackStatus.DENIED
    )
    denied_feedbacks_before = (
        await get_count_of_feedbacks(
            start_date_before, end_date_before, FeedbackStatus.DENIED
        )
        if start_date_before and end_date_before
        else 0
    )
    ...
    count_feedbacks_before = {
        ...
        FeedbackStatus.WAITING: all_feedbacks_before
        - (approved_feedbacks_before + denied_feedbacks_before),
    }
    ...
    count_income_money["AVERAGE_PRICE"] = (
        ...
        if count_income_money_total
        else 0
    )
    count_income_money_before["AVERAGE_PRICE"] = (
        (count_income_money_before["ALL"] / count_income_money_total_before)
        if count_income_money_total_before
        else 0
    )
    count_income_money["VAL"] = (
        count_income_money["ALL"] - count_expense_money["ALL"] * 100
    )
    count_income_money_before["VAL"] = (
        count_income_money_before["ALL"] - count_expense_money_before["ALL"] * 100
    )
    ...
    texts = {
        ...
        "summary_models": get_localization(
            language_code
        ).admin_statistics_summary_models(
            period=period,
            summary_products=summary_products,
            count_all_transactions=count_all_transactions,
            count_all_transactions_before=count_all_transactions_before,
...
async def handle_statistics_selection(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with ChatActionSender.typing(
        bot=callback_query.bot, chat_id=callback_query.message.chat.id
    ):
...
@statistics_router.callback_query(
    lambda c: c.data.startswith("statistics_write_transaction:")
)
async def handle_statistics_write_transaction_selection(
    ...
    if transaction_type == "back":
        ...
        await callback_query.message.edit_text(
            ...
            reply_markup=build_statistics_choose_service_keyboard(
                user_language_code, products, transaction_type
            ),
        )
...
@statistics_router.callback_query(
    lambda c: c.data.startswith("statistics_choose_service:")
)
async def handle_statistics_choose_service_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
@statistics_router.callback_query(
    lambda c: c.data.startswith("statistics_choose_currency:")
)
async def handle_statistics_choose_currency_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
@statistics_router.message(
    Statistics.waiting_for_statistics_service_quantity, ~F.text.startswith("/")
)
...
@statistics_router.message(
    Statistics.waiting_for_statistics_service_amount, ~F.text.startswith("/")
)
...
@statistics_router.message(
    Statistics.waiting_for_statistics_service_date, ~F.text.startswith("/")
)
