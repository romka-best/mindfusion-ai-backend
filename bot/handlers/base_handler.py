class BaseHandler:
    ...
    async def create_instance(  # noqa: PLR0913
        ...
        callback_query=None,
        delete_prev_msgs_num=0,
        delete_prev_msgs_auto=True,  # noqa: FBT002
    ):
        instance = cls(
    ...
    async def _delete_prev_msgs(self, start_msg_id=None):
        message_id = start_msg_id if start_msg_id else self.message.message_id

        await self.message.bot.delete_messages(
            self.message.chat.id,
            [
                message_id - offset
                for offset in range(self.delete_prev_msgs_num)
            ],
        )
