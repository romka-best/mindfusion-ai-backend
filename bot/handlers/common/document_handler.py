...
from bot.database.models.common import ClaudeGPTVersion, GeminiGPTVersion, Model, Quota
...
from bot.handlers.common.photo_handler import handle_album, handle_photo
...
async def document(message: Message, state: FSMContext, album: list[Message]):
    if len(album):
        ...
        or message.document.mime_type == "application/x-javascript"
        or message.document.mime_type == "text/javascript"
        or message.document.mime_type == "text/ecmascript"
        or message.document.mime_type == "application/x-python"
        or message.document.mime_type == "text/x-python"
        or message.document.mime_type == "text/x-script.phyton"
        or message.document.mime_type == "text/plain"
        or message.document.mime_type == "text/html"
        or message.document.mime_type == "text/css"
        or message.document.mime_type == "text/md"
        or message.document.mime_type == "text/csv"
        or message.document.mime_type == "text/xml"
        or message.document.mime_type == "text/rtf"
    ):
...
async def handle_document(message: Message, state: FSMContext, document_file: File):
    ...
    if (
        ...
        document_data_io = await message.bot.download_file(
            document_file.file_path, timeout=300
        )
        ...
        if user.current_model == Model.CLAUDE:
            await handle_claude(
                message, state, user, quota, [document_vision_filename], True
            )
