...
async def handle_video(message: Message, state: FSMContext, video_file: Video):
    ...
    need_exit = (
        ...
        or await is_messages_limit_exceeded(message, state, user, Quota.GEMINI_VIDEO)
    ...
    if user.current_model == Model.GEMINI_VIDEO:
        ...
        video_vision_path = (
            f"users/videos/{Quota.GEMINI_VIDEO}/{user_id}/{video_vision_filename}"
        )
        ...
        video_vision_filename = f"{uuid.uuid4()}.mp4"
        video_vision_path = (
            f"users/videos/{Quota.FACE_SWAP}/{user_id}/{video_vision_filename}"
        )
        video_vision = firebase.bucket.new_blob(video_vision_path)
        await video_vision.upload(video_data)
        video_link = firebase.get_public_url(video_vision_path)

        await handle_face_swap_video(
            message, state, user, video_link, video_file.duration
        )
    else:
        await message.reply(
