...
from bot.database.operations.face_swap_package.updaters import (
...
from bot.integrations.replicate_ai import (
    create_face_swap_image,
    create_photoshop_ai_image,
)
...
async def handle_photo(message: Message, state: FSMContext, photo_file: File):
    ...
    if current_state == Profile.waiting_for_photo.state:
        ...
        photo_data_io = await message.bot.download_file(
            photo_file.file_path, timeout=300
        )
        ...
        existing_blobs = await firebase.bucket.list_blobs(
        ...
        for used_face_swap_package in used_face_swap_packages:
            await update_used_face_swap_package(
                used_face_swap_package.id,
                {
                    "used_images": [],
                },
            )
        ...
        await processing_message.edit_text(
            get_localization(user_language_code).PROFILE_CHANGE_PHOTO_SUCCESS
        )
        ...
        if user.current_model == Model.FACE_SWAP:
            await handle_face_swap(
                message.bot, str(message.chat.id), state, str(message.from_user.id)
            )
        ...
        await message.answer(
            text=get_localization(
                user_language_code
            ).admin_catalog_create_role_confirmation(
            ...
            reply_markup=build_manage_catalog_create_role_confirmation_keyboard(
                user_language_code
            ),
        )
        ...
        await update_face_swap_package(
            face_swap_package.id, {"files": face_swap_package.files}
        )

        await message.answer(
            text=get_localization(user_language_code).ADMIN_FACE_SWAP_EDIT_SUCCESS
        )
        ...
        quota = (
            user.daily_limits[Quota.PHOTOSHOP_AI]
            + user.additional_usage_quota[Quota.PHOTOSHOP_AI]
        )
        if quota < 1:
        ...
        async with ChatActionSender.upload_photo(
            ...
            photo_path = (
                f"users/photoshop/{photoshop_ai_action_name}/{user_id}/{photo_name}"
            )
            ...
            result = await create_photoshop_ai_image(
                photoshop_ai_action_name, photo_link
            )
        ...
        or user.settings[user.current_model][UserSettings.VERSION]
        == ClaudeGPTVersion.V3_Sonnet
        or user.settings[user.current_model][UserSettings.VERSION]
        == ClaudeGPTVersion.V3_Opus
        or user.current_model == Model.GEMINI
        or user.current_model == Model.GROK
    ):
        if not (
            user.daily_limits[Quota.WORK_WITH_FILES]
            or user.additional_usage_quota[Quota.WORK_WITH_FILES]
        ):
            await message.answer(
                text=get_localization(
                    user_language_code
        ...
        need_exit = (
            ...
            or await is_messages_limit_exceeded(message, state, user, quota)
        ...
        or user.current_model == Model.STABLE_DIFFUSION
        or user.current_model == Model.FLUX
        or user.current_model == Model.LUMA_PHOTON
    ):
        current_time = time.time()
        ...
        if user.current_model == Model.MIDJOURNEY:
            ...
            await handle_stable_diffusion(
                message, state, user, user_quota, photo_vision_filename
            )
        ...
        if quota < quantity:
            ...
            async with ChatActionSender.upload_photo(
                ...
                try:
                    ...
                    background_path = (
                        f"users/backgrounds/{user_id}/{uuid.uuid4()}.{photo_extension}"
                    )
                    ...
                    result = await create_face_swap_image(
                        background_photo_link, user_photo_link
                    )
                    request = await write_request(
                        user_id=user_id,
                        processing_message_ids=[
                    ...
                    await message.answer_photo(
                        photo=URLInputFile(
                            photo_link, filename=photo_path, timeout=300
                        ),
        ...
        or user.current_model == Model.RUNWAY
        or user.current_model == Model.PIKA
        or user.current_model == Model.LUMA_RAY
    ):
        current_time = time.time()
    ...
    if (
        ...
        for file in album:
            if file.photo:
                ...
                file.document.mime_type.startswith("image") and file.document.thumbnail
            ):
        ...
        or user.current_model == Model.FLUX
        or user.current_model == Model.LUMA_PHOTON
        or user.current_model == Model.FACE_SWAP
        or user.current_model == Model.PHOTOSHOP_AI
        or user.current_model == Model.KLING
        or user.current_model == Model.RUNWAY
        or user.current_model == Model.PIKA
    ):
        await message.reply(
