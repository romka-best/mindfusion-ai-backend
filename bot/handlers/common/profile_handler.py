...
from aiogram.types import CallbackQuery, Message, URLInputFile, User as TelegramUser
...
async def handle_profile(
    message: Message, state: FSMContext, telegram_user: TelegramUser, is_edit=False
):
    ...
    if (
        ...
        or user.is_premium != telegram_user.is_premium
        or user.language_code != telegram_user.language_code
    ):
        await update_user(
            user.id,
            {
    ...
    if subscription:
        ...
        subscription_name = "🆓"
        renewal_date = (
            user.last_subscription_limit_update + timedelta(days=30)
        ).strftime("%d.%m.%Y")

    user_current_quota = get_quota_by_model(
...
async def handle_profile_change_photo(
...
async def handle_profile_gender_selection(
