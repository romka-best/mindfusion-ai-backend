...
from bot.database.models.common import Model, ModelType, Quota
...
@catalog_router.callback_query(
    lambda c: c.data.startswith("catalog_digital_employees:")
)
async def handle_catalog_digital_employees_selection(
    ...
    if (
        not user.daily_limits[Quota.ACCESS_TO_CATALOG]
        and not user.additional_usage_quota[Quota.ACCESS_TO_CATALOG]
    ):
        ...
        if keyboard_changed:
            ...
            await callback_query.message.reply_photo(
                photo=URLInputFile(role_photo_link, filename=role.photo, timeout=300),
                caption=role.translated_descriptions.get(user_language_code)
                or role.translated_descriptions.get(LanguageCode.EN),
                allow_sending_without_reply=True,
            )
...
@catalog_router.callback_query(
    lambda c: c.data.startswith("catalog_prompts_model_type:")
)
async def handle_catalog_prompts_model_type_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def handle_catalog_prompts_model_type(
    message: Message, user_id: str, state: FSMContext, is_edit=True
):
    user_language_code = await get_user_language(user_id, state.storage)
    user_data = await state.get_data()

    prompt_categories = await get_prompt_categories_by_model_type(
        user_data.get("prompt_model_type")
    )
...
async def handle_catalog_prompts_category_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def handle_catalog_prompts_category(
    message: Message, user_id: str, state: FSMContext
):
    user_language_code = await get_user_language(user_id, state.storage)
    user_data = await state.get_data()

    prompt_subcategories = await get_prompt_subcategories_by_category_id(
        user_data.get("prompt_category_id")
    )

    await message.edit_text(
        text=get_localization(user_language_code).CATALOG_PROMPTS_CHOOSE_SUBCATEGORY,
        reply_markup=build_catalog_prompt_subcategories_keyboard(
            user_language_code, prompt_subcategories
        ),
    )


@catalog_router.callback_query(
    lambda c: c.data.startswith("catalog_prompt_subcategory:")
)
async def handle_catalog_prompts_subcategory_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def handle_catalog_prompts_subcategory(
    message: Message, user_id: str, state: FSMContext
):
    user_language_code = await get_user_language(user_id, state.storage)
    user_data = await state.get_data()

    prompts = await get_prompts_by_subcategory_id(
        user_data.get("prompt_subcategory_id")
    )

    await message.edit_text(
        text=get_localization(user_language_code).catalog_prompts_choose_prompt(
            prompts
        ),
...
async def handle_catalog_prompt_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
...
async def handle_catalog_prompt(message: Message, user_id: str, state: FSMContext):
    ...
    await message.edit_text(
        text=get_localization(user_language_code).catalog_prompts_info_prompt(
            prompt, products
        ),
...
async def handle_catalog_prompt_chosen_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
    ...
    if action == "back":
        ...
        if action == "short":
            await state.update_data(
                prompt_text=prompt.short_prompts.get(user_language_code)
            )
        elif action == "long":
            await state.update_data(
                prompt_text=prompt.long_prompts.get(user_language_code)
            )
            ...
            for blob in blobs:
                ...
                blob_id = blob_name.split(".")[0]
                if blob_id and blob_id in prompt.product_ids:
                    media_caption = get_localization(
                        user_language_code
                    ).catalog_prompts_examples(products)
                    media.append(
                        InputMediaPhoto(
                            media=URLInputFile(
                                firebase.get_public_url(blob),
                                filename=blob_name,
                                timeout=300,
                            ),
...
@catalog_router.callback_query(
    lambda c: c.data.startswith("catalog_prompt_info_chosen:")
)
async def handle_catalog_prompt_info_chosen_selection(
    callback_query: CallbackQuery, state: FSMContext
):
    await callback_query.answer()
