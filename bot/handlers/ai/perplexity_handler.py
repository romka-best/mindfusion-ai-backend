...
from bot.database.models.common import Currency, Model, PerplexityGPTVersion, Quota
...
from bot.helpers.getters.get_history_without_duplicates import (
    get_history_without_duplicates,
)
    ...
    if user.current_model == Model.PERPLEXITY:
        await message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.PERPLEXITY
            ),
        )
    else:
...
async def handle_perplexity(
    message: Message, state: FSMContext, user: User, photo_filenames=None
):
    ...
    async with chat_action_sender(bot=message.bot, chat_id=message.chat.id):
        try:
            ...
            if (
                user.settings[user.current_model][UserSettings.VERSION]
                == PerplexityGPTVersion.Sonar
            ):
                input_price = (
                    response["input_tokens"] * PRICE_PERPLEXITY_SOLAR_INPUT_TOKEN
                )
                output_price = (
                    response["output_tokens"] * PRICE_PERPLEXITY_SOLAR_OUTPUT_TOKEN
                )
            elif (
                user.settings[user.current_model][UserSettings.VERSION]
                == PerplexityGPTVersion.Sonar_Pro
            ):
                input_price = (
                    response["input_tokens"] * PRICE_PERPLEXITY_SOLAR_PRO_INPUT_TOKEN
                )
                output_price = (
                    response["output_tokens"] * PRICE_PERPLEXITY_SOLAR_PRO_OUTPUT_TOKEN
                )

            product = await get_product_by_quota(Quota.PERPLEXITY)

            total_price = round(
                input_price + output_price + PRICE_PERPLEXITY_REQUEST, 6
            )
            ...
            message_role, message_content = (
                ...
                response_message_with_citations,
            )
            ...
            await create_new_message_and_update_user(
                transaction, message_role, message_content, user, Quota.PERPLEXITY
            )
            ...
            if user.settings[user.current_model][UserSettings.TURN_ON_VOICE_MESSAGES]:
                ...
                footer_text = (
                    ...
                    and user.daily_limits[Quota.GROK_2] != float("inf")
                    else ""
                )
                reply_markup = build_continue_generating_keyboard(user_language_code)
