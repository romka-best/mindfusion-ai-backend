...
from bot.database.models.common import ClaudeGPTVersion, Currency, Model, Quota
...
async def handle_claude_choose_selection(
    ...
    if (
        ...
        and chosen_version == user.settings[Model.CLAUDE][UserSettings.VERSION]
    ):
        await callback_query.message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.CLAUDE
            ),
        )
    else:
...
async def handle_claude(
    ...
    if filenames and len(filenames):
        await write_message(
            user.current_chat_id, "user", user.id, text, True, filenames
        )
    else:
    ...
    system_prompt = role.translated_instructions.get(
        user_language_code
    ...
    for sorted_message in sorted_messages:
        ...
        if sorted_message.photo_filenames and (
            can_work_with_photos or can_work_with_documents
        ):
            for photo_filename in sorted_message.photo_filenames:
                ...
                data = await asyncio.to_thread(
                    lambda: base64.b64encode(response_content).decode("utf-8")
                )
                content.append(
                    {
                        "type": "image"
...
async def handle_claude_3_sonnet_example(
    ...
    try:
        ...
        if (
            ...
            and user.current_model == Model.CLAUDE
            and user.settings[user.current_model][UserSettings.SHOW_EXAMPLES]
            and user.daily_limits[Quota.CLAUDE_3_HAIKU] + 1 in [3, 7]
            and (current_date - user.last_subscription_limit_update).days <= 3
        ):
            history = get_history_without_duplicates(history)

            response = await get_response_message(
                ClaudeGPTVersion.V3_Sonnet, system_prompt, history
            )
            response_message = response["message"]
