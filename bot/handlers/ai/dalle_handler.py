...
from bot.integrations.open_ai import get_cost_for_image, get_response_image
    ...
    if user.current_model == Model.DALL_E:
        await message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.DALL_E
            ),
        )
    else:
    ...
    async with ChatActionSender.upload_photo(bot=message.bot, chat_id=message.chat.id):
        try:
            ...
            footer_text = (
                ...
                if user.settings[Model.DALL_E][UserSettings.SHOW_USAGE_QUOTA]
                and user.daily_limits[Quota.DALL_E] != float("inf")
                else ""
            )
