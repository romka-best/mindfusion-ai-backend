...
from bot.database.models.common import Currency, DeepSeekVersion, Model, Quota
...
async def handle_deep_seek_choose_selection(
    ...
    if (
        ...
        and chosen_version == user.settings[Model.DEEP_SEEK][UserSettings.VERSION]
    ):
        await callback_query.message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.DEEP_SEEK
            ),
        )
    else:
...
async def handle_deep_seek_r1_example(
    ...
    try:
        ...
        if (
            ...
            and user.current_model == Model.DEEP_SEEK
            and user.settings[user.current_model][UserSettings.SHOW_EXAMPLES]
            and user.daily_limits[Quota.DEEP_SEEK_R1] + 1 in [3, 7]
