...
from bot.integrations.google import get_response_video_summary
    ...
    if user.current_model == Model.GEMINI_VIDEO:
        await message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.GEMINI_VIDEO
            ),
        )
    else:
...
async def handle_gemini_video(
    message: Message, state: FSMContext, user: User, video_link: str
):
        ...
        try:
            ...
            if user.settings[user.current_model][UserSettings.TURN_ON_VOICE_MESSAGES]:
                ...
                footer_text = (
                    ...
                    and user.daily_limits[Quota.GEMINI_VIDEO] != float("inf")
                    else ""
                )
