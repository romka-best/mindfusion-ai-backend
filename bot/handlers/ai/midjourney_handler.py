    ...
    if user.current_model == Model.MIDJOURNEY:
        await message.answer(
            ...
            reply_markup=build_switched_to_ai_keyboard(
                user_language_code, Model.MIDJOURNEY
            ),
        )
    else:
...
async def handle_midjourney(
    ...
    async with ChatActionSender.upload_photo(bot=message.bot, chat_id=message.chat.id):
        quota = (
            user.daily_limits[Quota.MIDJOURNEY]
            + user.additional_usage_quota[Quota.MIDJOURNEY]
        )
...
async def handle_midjourney_example(
    user: User, user_language_code: LanguageCode, prompt: str, message: Message
):
